...
using QuickCode.DemoAi.IdentityModule.Application.Features.KafkaEvent;
using QuickCode.DemoAi.IdentityModule.Domain.Enums;

namespace QuickCode.DemoAi.IdentityModule.Api.Controllers
{
    public partial class KafkaEventsController : QuickCodeBaseApiController
    {
        ...
        [ProducesResponseType(StatusCodes.Status404NotFound, Type = typeof(string))]
        [ProducesResponseType(StatusCodes.Status400BadRequest, Type = typeof(string))]
        public async Task<IActionResult> GetActiveKafkaEventsAsync(bool kafkaEventIsActive)
        {
            var response = await mediator.Send(new GetActiveKafkaEventsQuery(kafkaEventIsActive));
            if (HandleResponseError(response, logger, "KafkaEvent", $"") is {} responseError)
                return responseError;
            return Ok(response.Value);
        }
