...
namespace QuickCode.DemoAi.ProductCatalogModule.Api.Controllers
{
    public partial class CategoriesController : QuickCodeBaseApiController
    {
        ...
        [ProducesResponseType(StatusCodes.Status404NotFound, Type = typeof(string))]
        [ProducesResponseType(StatusCodes.Status400BadRequest, Type = typeof(string))]
        public async Task<IActionResult> SearchByNameAsync(string categoryName, int? page, int? size)
        {
            if (page < 1)
            {
                var pageNumberError = $"Page Number must be greater than 1";
            ...
            var response = await service.SearchByNameAsync(categoryName, page, size);
